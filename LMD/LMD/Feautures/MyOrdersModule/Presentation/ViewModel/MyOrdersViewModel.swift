...
class MyOrdersViewModel: ObservableObject {
    ...
    @Published private(set) var ordersData: OrdersData?
    @Published private(set) var pagination: Paginations?
    ...
    // UseCases
    private let getMyOrders: GetMyOrdersUseCase
    private let updateOrderStatus: UpdateOrderStatusUseCase
    ...
    private func loadOrders(page: Int, reset: Bool) async {
        ...
        do {
            let result = try await getMyOrders.execute(page: page, limit: 100)
    ...
    func updateOrderStatues(orderId: String, statusId: Int, assignedAgentId: String? = nil) async {
        ...
        do {
            let result = try await updateOrderStatus.execute(
                orderId: orderId,
                statusId: statusId,
...
//            }
    ...
    func getAllUsers() async {
        ...
        do {
            ...
            let result = try await getAllUsers.execute()
            self.users = result
